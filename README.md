...
POST /api/generate  { "topic": "فوائد شرب الماء" }
        ...
        ├── Kokoro TTS   →  صوت عربي مجتمعي (تجريبي) — اختياري
...
curl -X POST http://localhost:8282/api/generate \
  ...
  -d '{
    ...
    "ttsType": "edge",                    // محرك الصوت: edge (بدون مفتاح) | google (مفتاح) | kokoro (عربي مجتمعي تجريبي) | piper (عربي محلي)
...
│   │   ├── kokoro_tts.js     # Kokoro TTS (عربي مجتمعي تجريبي)
...
- **الصوت متصل**: السرد يُولَّد بنداء TTS **واحد للنص الكامل** ثم يُقسَّم لكل مشهد — فيسرٍ طبيعي بلا فجوات صمت بين المشاهد. المحركات `kokoro`/`piper` تُقارِب توقيت الكلمات (ليست دقيقة كـ Edge).
- **محركات صوت إضافية**: `kokoro` (عربي مجتمعي تجريبي — قد يحتاج نموذجاً مخصصاً) و `piper` (عربي حقيقي `ar_JO-kareem-medium` يُنزَّل تلقائياً). عند فشل أي محرك يتراجع النظام تلقائياً إلى Edge لضمان توليد الفيديو.
